...
public sealed partial class NivalisLobbyControl : Control
{
    ...
    private readonly Dictionary<Button, string> _pendingPerkIcons = new();
    ...
    public Control LoadoutNavButton => LoadoutButton;
    ...
    internal static Texture GenerateScanlines()
    {
        const int width = 64;
        const int height = 6;
    ...
    public void SetCustomizationOpen(bool open)
    {
        _customizationOpen = open;
        DimNavButtons(open);

        CharacterViewport.Visible = !open;
    }
    ...
    private readonly Dictionary<Button, TextureRect> _perkIconRects = new();
    ...
    private Button MakePerkTileButton(PerkInfo perk)
    {
        ...
        _perkIconRects[btn] = iconRect;
        _pendingPerkIcons[btn] = perk.Icon;
        return btn;
    }

    private void EnsurePerkIcons()
    {
        if (_pendingPerkIcons.Count == 0)
            return;

        var sprite = _entityManager.System<SpriteSystem>();
        foreach (var (btn, state) in _pendingPerkIcons)
        {
            if (!_perkIconRects.TryGetValue(btn, out var rect))
                continue;

            rect.Texture = sprite.Frame0(new SpriteSpecifier.Rsi(
                new ResPath(PerkIconDir),
                state));
        }

        _pendingPerkIcons.Clear();
    }
    ...
    private void OnPerkPick(Button btn)
    {
        ...
        var selectedId = _selectedPerk != null
            ? new ProtoId<NivalisPerkPrototype>(_selectedPerk.Name)
            : default(ProtoId<NivalisPerkPrototype>?);
        _netManager.SendSystemNetworkMessage(new NivalisPerkSelectedMessage
        {
            Perk = selectedId,
        });
